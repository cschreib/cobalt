...
namespace server_state {
    ...
    configure::configure(server_instance& serv) :
        ...
        pool_ << net_.send_request(client::netcom::server_actor_id,
            make_packet<request::server::configure_list_generators>(),
            [this](const client::netcom::request_answer_t<request::server::configure_list_generators>& msg) {
                if (!msg.failed) {
                    available_generators_ = std::move(msg.answer.gens);
                }
            }
        );
    }
    ...
    void configure::register_lua(sol::state& lua) {
        ...
        ctbl.set_function("get_parameter_type", [this](std::string key) {
            std::string val;
            if (!config_.get_value(config_meta_header+key+".type", val)) {
                val = "string";
            }
            return val;
        });
        ctbl.set_function("get_parameter_range", [this](std::string key) {
            ...
            if (config_.get_value(config_meta_header+key+".min_value", minval)) {
                retmin.set(minval);
            }
            if (config_.get_value(config_meta_header+key+".max_value", maxval)) {
                retmax.set(maxval);
            }
